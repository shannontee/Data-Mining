...
text_file = sc.textFile(input_file).filter(lambda x: x != "user_id,business_id").repartition(2)

if case_num == 1:
	baskets = text_file.map(lambda x: (x.split(",")[0], [x.split(",")[1]])).reduceByKey(lambda a,b: a+b).map(lambda x: (x[0], sorted(set(x[1]))))

else:
	baskets = text_file.map(lambda x: (x.split(",")[1], [x.split(",")[0]])).reduceByKey(lambda a,b: a+b).map(lambda x: (x[0], sorted(set(x[1]))))
...
def phase1(subset, support, num_baskets):
	...
	candidate_itemsets = [(k,) for k in itemsets]
